 ...
 */
export const downgradeToPlan = onCall(async (request) => {
  logger.info("🔽 downgradeToPlan function triggered");

  // Check if user is authenticated
  if (!request.auth) {
    logger.warn("Unauthenticated user tried to call downgradeToPlan.");
  ...
  logger.info(`User ${userId} is downgrading to plan: ${planId}`);

  if (planId !== 'free') {
    throw new HttpsError('invalid-argument', 'This function only handles downgrades to free plan.');
  }
  ...
  try {
    ...
    logger.info(`Found ${qrCodesSnapshot.size} QR codes for user ${userId}`);
    ...
    if (qrCodesSnapshot.size > 1) {
      ...
      logger.info(`Keeping QR code: ${qrToKeep.id}`);
      logger.info(`Deleting ${qrsToDelete.length} QR codes`);
      ...
      logger.info(`Successfully deleted ${qrsToDelete.length} QR codes`);
    } else if (qrCodesSnapshot.size === 0) {
      logger.info('User has no QR codes to delete');
    } else {
      logger.info('User has only 1 QR code, no deletion needed');
    }
    ...
    if (!subscriptionsSnapshot.empty) {
      ...
      logger.info('Updated subscription to free plan');
    } else {
      ...
      logger.info('Created new free subscription');
    }
    ...
    throw new HttpsError('internal', 'Failed to downgrade plan');
  }
});
